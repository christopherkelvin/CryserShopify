import item from "../assets/images/ShopItem.jpeg";
import Veg from "../assets/images/VegLogo.png";
export function ItemsMiddle() {
  return (
    <div className=" m-12">
      <div className=" min-h-72 bg-white max-w-60 overflow-hidden relative rounded-[25px]">
        <div className="absolute h-[55px] w-[67px] overflow-hidden p-[3px] top-[120px] left-[85px] bg-white rounded-2xl">
          <div className=" bg-pink-200 w-full h-full rounded-2xl flex justify-center pt-1/2">
            <img src={Veg} alt="" className=" h-12" />
          </div>
        </div>
        <img src={item} className="h-36 w-full" alt="" />
        <div className=" mt-10 flex flex-col items-center">
          <h1 className=" font-bold text-[16px]">Green Vegetables</h1>
          <div className=" font-sans font-bold text-black/50">Tsh 5,000</div>
          <div className=" mt-1">
            <div className=" bg-black/40 h-[1px] w-60" />
            
          </div>
        </div>
      </div>
    </div>
  );
}
